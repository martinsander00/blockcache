...
async fn main() {
    ...
    tokio::spawn(async move {
        update_cache_periodically(db_client_clone, cache_clone).await;
    });
...
async fn update_cache_periodically(
    ...
    loop {
        ...
        // For each pool address you're interested in
        let pool_addresses = vec!["6d4UYGAEs4Akq6py8Vb3Qv5PvMkecPLS1Z9bBCcip2R7".to_string()];
